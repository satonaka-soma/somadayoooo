 ...
 * スタンプカード v10.2（完全版）
 * - v10.1フル機能をベース
 * - 追加: カード削除ボタン、ご褒美削除ボタン、テーマ4種対応、ポイント表示強調
 */
...
const Svg = {
  leaf: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      <path
        d="M56 8C38 10 22 18 14 30S8 56 8 56s14 2 26-6S54 26 56 8Z"
        fill="#10b981"
      />
      <path
        d="M32 16c0 20-4 28-20 40"
        stroke="#065f46"
        strokeWidth="3"
        fill="none"
      />
  ...
  paw: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      ...
      <circle cx="44" cy="20" r="6" fill="#0ea5e9" />
      <circle cx="28" cy="12" r="5" fill="#38bdf8" />
      <circle cx="36" cy="12" r="5" fill="#38bdf8" />
      <ellipse cx="32" cy="40" rx="14" ry="10" fill="#0ea5e9" />
    </svg>
  ),
  flower: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
  ...
  frog: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      <circle cx="22" cy="20" r="8" fill="#34d399" />
      <circle cx="42" cy="20" r="8" fill="#34d399" />
      <ellipse cx="32" cy="40" rx="18" ry="14" fill="#10b981" />
      <circle cx="22" cy="20" r="3" fill="#111" />
  ...
  gift: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      <rect
        ...
        fill="#ef4444"
      />
      <rect x="30" y="16" width="4" height="40" fill="#fbbf24" />
      <rect x="8" y="34" width="48" height="4" fill="#fbbf24" />
      <path
        d="M24 20c0-6 8-8 8-2 0-6 8-4 8 2"
        stroke="#fbbf24"
        strokeWidth="4"
        fill="none"
      />
  ...
  car: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      <rect
        ...
        fill="#3b82f6"
      />
      <path d="M16 28l8-8h16l8 8z" fill="#60a5fa" />
      <circle cx="22" cy="46" r="6" fill="#111" />
      <circle cx="42" cy="46" r="6" fill="#111" />
    </svg>
  ),
  bouquet: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      ...
      <path d="M32 22v28" stroke="#16a34a" strokeWidth="3" />
      <path d="M20 34l12 16M44 34L32 50" stroke="#16a34a" strokeWidth="3" />
    </svg>
  ),
  gem: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      <polygon
        points="16,24 32,8 48,24 40,48 24,48"
        fill="#22d3ee"
      />
      <path
        d="M32 8L24 48M32 8l8 40"
        stroke="#0891b2"
        strokeWidth="2"
        fill="none"
      />
  ...
  party: (p) => (
    <svg viewBox="0 0 64 64" {...p}>
      <path d="M10 52l14-30 30 14z" fill="#f59e0b" />
      <g fill="#f43f5e">
  ...
  capsuleTop: (p) => (
    <svg viewBox="0 0 120 80" {...p}>
      <path
        d="M10 40a30 30 0 0 1 30-30h40a30 30 0 0 1 30 30"
        fill="#fde68a"
        stroke="#f59e0b"
        strokeWidth="4"
      />
  ...
  capsuleBottom: (p) => (
    <svg viewBox="0 0 120 80" {...p}>
      <path
        d="M10 40a30 30 0 0 0 30 30h40a30 30 0 0 0 30-30"
        fill="#fca5a5"
        stroke="#ef4444"
        strokeWidth="4"
      />
...
function WoodBackground() {
  return (
    <div
      className="pointer-events-none absolute inset-0 -z-10 bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')] bg-repeat"
    />
  );
}
...
export default function App() {
  ...
  const [habits, setHabits] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("stampcard_v10") || "[]");
    } catch {
      return [];
    }
  });
  const [theme, setTheme] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("stampcard_theme") || '"island"');
    } catch {
      return "island";
    }
  });
  ...
  const [gachaHistory, setGachaHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("stampcard_gacha") || "{}");
    } catch {
      return {};
    }
  });
  const [pointsSpent, setPointsSpent] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("stampcard_points_spent") || "0");
    } catch {
      return 0;
    }
  });
  const [redeemHistory, setRedeemHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("stampcard_redeem_hist") || "[]");
    } catch {
      return [];
    }
  });
    ...
    setHabits((prev) =>
      prev.map((h) =>
        h.id === habitId
          ? {
              ...h,
              rewards: [...(h.rewards || []), { threshold, label }].sort((a, b) => a.threshold - b.threshold),
            }
          : h
      )
    );
  ...
  const removeReward = (habitId, idx) =>
    setHabits((prev) =>
      prev.map((h) =>
        h.id !== habitId
          ? h
          : { ...h, rewards: h.rewards.filter((_, i) => i !== idx) }
      )
    );
  // ガチャ
  const spinGacha = () => {
    ...
    setTimeout(() => {
      ...
      setGachaHistory((prev) => ({
        ...prev,
        [k]: { outcome, bonus, at: new Date().toISOString() },
      }));
      if (outcome === "jackpot")
        setBanner({
          text: "超大当たり!! +10pt",
        ...
        setBanner({ text: "あたり！ +1pt", iconKey: "gift" });
      else setBanner(null);
    }, 1100);
  };

  const redeem = (label, cost) => {
    ...
    setRedeemHistory((prev) =>
      [{ id: uid(), at: new Date().toISOString(), label, cost }, ...prev].slice(0, 200)
    );
    ...
    <div className="min-h-screen relative text-gray-900 overflow-x-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <select
              ...
              className="px-2 py-1 rounded-lg border bg-white/80"
            >
      ...
      <div className="text-center py-3 bg-white/60 backdrop-blur border-b">
        ...
        <div className="text-xs text-gray-500">
          使用済: {pointsSpent} / 使用可能: <b>{availablePoints}</b>
        </div>
      </div>
      ...
      {banner && (
        <div
          className={`fixed top-16 left-1/2 -translate-x-1/2 z-30 px-4 py-2 rounded-2xl shadow-xl border bg-amber-50/95 flex items-center gap-2 ${
      ...
      {view === "cards" ? (
        <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
          ...
          <div className="bg-white/80 backdrop-blur rounded-2xl shadow-sm border p-4 mb-4 flex flex-col items-center">
            ...
            {gachaHistory[todayKey()] ? (
              <div className="flex items-center gap-2 text-sm">
                <span>結果：</span>
                {gachaHistory[todayKey()].outcome === "miss" && (
                  <span className="px-2 py-1 rounded-lg bg-gray-100 border">残念…また明日！</span>
                )}
                {gachaHistory[todayKey()].outcome === "hit" && (
                  <span className="px-2 py-1 rounded-lg bg-amber-100 border">あたり！+1pt</span>
                )}
                {gachaHistory[todayKey()].outcome === "jackpot" && (
                  <span className="px-2 py-1 rounded-lg bg-pink-100 border">超大当たり！！+10pt</span>
                )}
              </div>
            ) : (
              <button
                onClick={spinGacha}
                className={`relative w-44 h-44 rounded-full shadow-xl overflow-hidden ${
                  reduceMotion ? "" : "animate-spin-slow-snap"
                }`}
              >
                <div className="absolute inset-x-0 top-3 flex items-center justify-center">
                  <Svg.capsuleTop className="w-32 h-16" />
                </div>
                <div className="absolute inset-x-0 bottom-3 flex items-center justify-center">
                  <Svg.capsuleBottom className="w-32 h-16" />
                </div>
                <div className="absolute inset-0 flex items-center justify-center text-white font-bold text-xl mix-blend-overlay">
                  回す！
                </div>
              </button>
            )}
            <div className="text-xs text-gray-600 mt-2">
              ポイント：スタンプ + ボーナス（結果は自動加算）
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur rounded-2xl shadow-sm border p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <input
                ...
                placeholder="名前（例：英語シャドーイング）"
                className="px-3 py-2 rounded-xl border w-full"
              />
              <input
                value={newRule}
                onChange={(e) => setNewRule(e.target.value)}
                placeholder="ルール（例：1日15分やったら押す）"
                className="px-3 py-2 rounded-xl border w-full"
              />
              <button
                onClick={addHabitFromForm}
          ...
          <div className="flex flex-wrap items-center justify-between mb-3 gap-2">
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl border bg-white/70 hover:bg-white"
              >
                ← 前月
              </button>
          ...
          <div className="space-y-6">
            {habits.length === 0 && (
              <div className="text-gray-700 text-sm bg-white/70 rounded-xl p-3 inline-block">
                まずカードを作成してください。
              </div>
            )}
            {habits.map((h) => (
      ...
      <footer className="relative z-10 max-w-5xl mx-auto px-4 py-8 text-xs text-gray-700">
...
function HabitCard({
  ...
  const streakToday = useMemo(
    () => computeStreakFromDate(stamps, restDays, todayKey()),
    [stamps, restDays]
  );
    ...
    () =>
      (rewards || [])
        .map((r) => ({
          ...
          can: total >= r.threshold,
          remain: Math.max(0, r.threshold - total),
        }))
    ...
    <div className="rounded-3xl border shadow-sm overflow-hidden bg-white/80 backdrop-blur">
      ...
      <div className="p-4 flex items-center gap-3">
        ...
        <button
          ...
          className="ml-2 px-2 py-1 text-xs rounded-lg border text-red-600 hover:bg-red-50"
        >
          削除
        </button>
      </div>
      ...
      <div className="px-4 pb-4">
        <div className="rounded-3xl p-3 border bg-[linear-gradient(135deg,#f7f5ef,#efe9dc)] shadow-inner">
          <div className="grid grid-cols-7 gap-1 text-[12px] text-gray-700 mb-1 font-medium">
            {["月", "火", "水", "木", "金", "土", "日"].map((w) => (
          ...
          <div className="grid grid-cols-7 gap-1">
            {days.map((d, idx) => {
              ...
              const iconKey = special
                ? special.icon
                : marked
                ? NORMAL_ICONS[idx % NORMAL_ICONS.length]
                : null;
              ...
              return (
                <div key={k} className="relative">
                  ...
                  >
                    <span
                      className={`absolute top-1 left-1 text-[14px] ${
                        marked ? "text-white/90" : "text-emerald-800/70"
                      }`}
                    >
                      {d.getDate()}
                    </span>
                    {iconKey ? (
                      <Icon
                        iconKey={iconKey}
                        className={`w-12 h-12 ${marked ? "animate-sparkle" : ""}`}
                      />
                    ) : (
                      !rest && (
                        <Icon iconKey="leaf" className="w-9 h-9 opacity-20" />
                      )
                    )}
                    {special && (
                      <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[11px] font-bold px-1.5 py-0.5 rounded-full bg-amber-200 text-amber-800 border border-amber-300 shadow">
                        {special.label}
                      </span>
                    )}
                    {rest && (
                      <span className="absolute bottom-1 right-1 text-[11px] px-1 rounded bg-white/90 border">
                        休息
                      </span>
                    )}
                  </button>
                  {restAllowed && !isToday && (
                    <button
                      onClick={() => onToggleRest(k)}
                      className="absolute -bottom-2 left-1/2 -translate-x-1/2 text-[10px] px-1.5 py-0.5 rounded-full bg-white border shadow hover:bg-gray-50"
                    >
                      休息切替
                    </button>
                  )}
          ...
          <ul className="space-y-1">
            {rewardStatuses.map((rs, i) => (
              <li key={i} className="flex items-center justify-between">
                ...
                {rs.can ? (
                  <span className="text-emerald-600 font-semibold">交換可能！</span>
                ) : (
                  <span className="text-gray-700">あと {rs.remain} スタンプ！</span>
                )}
              </li>
            ))}
          </ul>
      ...
      <div className="bg-white/80 backdrop-blur rounded-2xl shadow-sm border p-4 mb-6 grid md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="text-gray-700 mt-1">
            総pt: <b>{totalPoints}</b> / 使用済: {pointsSpent} / 使用可能: <b>{availablePoints}</b>
          </div>
...
function RewardSettings({ habits, addReward, removeReward }) {
  ...
  useEffect(() => {
    if (!habits.find((x) => x.id === selected)) setSelected(habits[0]?.id || null);
  }, [habits, selected]);
          ...
          <>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <input
                value={label}
                onChange={(e) => setLabel(e.target.value)}
                className="px-3 py-2 rounded-xl border w-72"
                placeholder="ご褒美名"
              />
              <button
                onClick={() => {
