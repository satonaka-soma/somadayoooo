 ...
 * - 背景テーマ4種（island / notebook / wood / night）を正しく描画
 * - カード削除／ご褒美削除ボタン実装
 */
...
function App() {
  ...
  const bonusPoints = useMemo(() => Object.values(gachaHistory).reduce((s, r) => s + (r && r.bonus ? r.bonus : 0), 0), [gachaHistory]);
  ...
  const stampToday = (habitId) => {
    ...
    const newStamps = { ...(h && h.stamps ? h.stamps : {}), [k]: !(h && h.stamps && h.stamps[k]) };
    const streak = computeStreakFromDate(newStamps, (h && h.restDays) ? h.restDays : {}, k);
  ...
  const spinGacha = () => {
    ...
    setTimeout(() => {
      ...
      setGachaHistory(prev => { const n = Object.assign({}, prev); n[k] = { outcome, bonus, at: new Date().toISOString() }; return n; });
...
/* ========= sub components ========= */
function HabitCard({ habit, days, currentMonth, onStampToday, onToggleRest, onDeleteCard }) {
  ...
  return (
    <div className="rounded-3xl border shadow-sm overflow-hidden bg-white/80 backdrop-blur">
      ...
      <div className="p-4 flex items-center gap-3">
        ...
        <button onClick={onStampToday} className={`px-4 py-2 rounded-xl border ml-2 ${stamps && stamps[todayKey()] ? 'bg-emerald-600 text-white' : 'bg-white hover:bg-emerald-50'}`}>{stamps && stamps[todayKey()] ? 'できた！✓' : 'できた！'}</button>
...
function RewardSettings({ habits, addReward, removeReward }) {
  const [selected, setSelected] = useState(habits[0] ? habits[0].id : null);
  const [label, setLabel] = useState("");
  const [threshold, setThreshold] = useState(10);
  const h = habits.find(x => x.id === selected);
  useEffect(() => { if (!habits.find(x => x.id === selected)) setSelected(habits[0] ? habits[0].id : null); }, [habits, selected]);
  return (
    <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
      <div className="bg-white/80 backdrop-blur rounded-2xl shadow-sm border p-4 mb-4">
