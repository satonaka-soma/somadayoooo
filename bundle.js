...
/**
 * スタンプカード v10.2+ 完全版（GHP直置き・UMD）
 * - 背景テーマ4種（island / notebook / wood / night）を確実描画（白抜け防止）
 * - カード「作成」ボタン（押すまで作らない）
 * - カード削除 / ご褒美削除
 * - 各カードに「交換可能なご褒美」＋「交換！」ボタン（その場で交換・全体ポイントから引落）
 * - ポイントを大きく強調
 * - ガチャは常に中央配置、結果はボーナス加算（1日1回）
 * - 休息日（当日/翌日不可・二日後以降）、連続日数、ストリーク演出、コンフェッティ
 * - localStorage 永続化（keys: stampcard_v10, stampcard_theme, stampcard_gacha, stampcard_points_spent, stampcard_redeem_hist）
 */
  ...
  return (
    <div className="pointer-events-none absolute inset-0 -z-10">
      <div className="absolute inset-0" style={{
        background: "linear-gradient(to bottom, #bae6fd, #e0f2fe 40%, #d1fae5 100%)"
      }} />
...
function NightBackground() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 -z-10" style={{
      background: "linear-gradient(to bottom, #1e1b4b, #0f172a 60%, #000)"
    }}>
      {stars.map(i => (
        <span key={i} className="absolute rounded-full bg-white"
          style={{ width: 2, height: 2, top: `${Math.random()*100}%`, left: `${Math.random()*100}%`, opacity: 0.8 }} />
      ))}
      <div className="absolute inset-0" style={{
        background: "radial-gradient(circle at 60% 20%, rgba(255,255,255,.15), transparent 40%)"
      }} />
    </div>
  );
}
...
function WoodBackground() {
  // 外部テクスチャは読み込めない環境があるので淡い木目風グラデに差し替え（安定）
  return (
    <div className="pointer-events-none absolute inset-0 -z-10"
      style={{
        background: "linear-gradient(135deg, #f8f5f0, #f3ece3 35%, #efe5d8 70%)"
      }}
    />
  );
}
...
function App() {
  ...
  // 「カード内」から交換（そのカードのご褒美 → 全体ポイントから引き落とし）
  const redeemFromCard = (habitId, rewardIndex) => {
    const h = habits.find(x => x.id === habitId);
    if (!h) return;
    const r = (h.rewards || [])[rewardIndex];
    if (!r) return;
    ...
    setBanner({ text: `「${r.label}」を受け取り！`, iconKey: 'party', effect: 'confetti' });
  ...
  return (
    <div className="min-h-screen relative text-gray-900 overflow-x-hidden" style={{ backgroundColor: "#eef5f2" }}>
      {/* 背景（テーマ反映） */}
        ...
        <div className="max-w-5xl mx-auto px-3 py-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="px-3 py-1.5 rounded-2xl border bg-white/80 text-sm md:text-base">
                <span className="opacity-70">総pt</span> <b className="text-xl md:text-2xl align-middle">{totalPoints}</b>
                <span className="mx-1 opacity-50">/</span>
                <span className="opacity-70">可</span> <b className="text-xl md:text-2xl align-middle text-emerald-700">{availablePoints}</b>
              </div>
            </div>
          </div>
          ...
          <div className="mt-2 flex gap-2 overflow-x-auto no-scrollbar flex-nowrap items-center">
            ...
            <label className="flex items-center gap-1 text-xs whitespace-nowrap ml-1">
      ...
      {view === 'cards' ? (
        <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
          {/* ガチャ（常に中央寄せ & 可変サイズ） */}
          <div className="bg-white/85 backdrop-blur rounded-2xl shadow-sm border p-4 mb-4">
          ...
          {/* 新規カード（作成ボタン） */}
          <div className="bg-white/85 backdrop-blur rounded-2xl shadow-sm border p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <button onClick={createByButton} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:opacity-90">作成</button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between mb-3 gap-2">
            ...
            <div className="text-sm bg-white/80 rounded-xl px-3 py-1.5 border">
              総pt: <b className="text-lg align-middle">{totalPoints}</b> / 使用済: {pointsSpent} / 使用可能: <b className="text-lg align-middle text-emerald-700">{availablePoints}</b>
            </div>
          </div>
  ...
  return (
    <div className="rounded-3xl border shadow-sm overflow-hidden bg-white/80 backdrop-blur">
      ...
      <div className="p-4 flex items-center gap-3">
        ...
        <button onClick={onDeleteCard} className="ml-2 px-2 py-1 text-xs rounded-lg border text-red-600 hover:bg-red-50">削除</button>
      </div>
      ...
      <div className="px-4 pb-4">
        <div className="rounded-3xl p-3 border bg-[linear-gradient(135deg,#f7f5ef,#efe9dc)] shadow-inner">
          ...
          {/* 交換可能なご褒美（カード直下） */}
          <div className="mt-4 rounded-2xl border bg-white/80 p-2">
            ...
            {exchangeable.length === 0 ? (
              ...
              <ul className="space-y-1">
                {exchangeable.map(r => (
                  <li key={r.i} className="flex items-center justify-between text-sm">
                    <span>{r.label}（{r.threshold}pt）</span>
                    <button onClick={() => onRedeem(r.i)} className="px-2 py-1 rounded-lg bg-emerald-600 text-white hover:opacity-90">交換！</button>
                  </li>
                ))}
              </ul>
            )}
...
function ShopView({ totalPoints, availablePoints, pointsSpent, setPointsSpent, redeemHistory, redeem }) {
  ...
  return (
    <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
      <div className="bg-white/85 backdrop-blur rounded-2xl shadow-sm border p-4 mb-6 grid md:grid-cols-2 gap-4">
        <div>
          <div className="font-semibold">ご褒美ポイント</div>
          <div className="text-gray-700 mt-1">総pt: <b className="text-xl align-middle">{totalPoints}</b> / 使用済: {pointsSpent} / 使用可能: <b className="text-xl align-middle text-emerald-700">{availablePoints}</b></div>
      ...
      <div className="bg-white/85 backdrop-blur rounded-2xl shadow-sm border p-4">
                  ...
                  <tbody>
                    {(h.rewards || []).sort((a, b) => a.threshold - b.threshold).map((r, i) => (
                      <tr key={i} className="border-t">
                        ...
                        <td className="py-2"><button onClick={() => removeReward(h.id, i)} className="px-2 py-1 rounded-lg border text-red-600 hover:bg-red-50">削除</button></td>
                      </tr>
                    ))}
