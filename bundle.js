...
/**
 * スタンプカード v10.1（ビルド修正版 / GHP直置き対応）
 * - import/export を撤去（UMDの React/ReactDOM を使用）
 * - そのまま JSX で書き、Babel Standalone で実行
 */
...
// ========== 背景・装飾 ==========
function IslandBackground({ reduceMotion }) {
  return (
...
function App() {
  ...
  return (
    <div className="min-h-screen relative text-gray-900 overflow-x-hidden">
      {/* 背景 */}
      {theme === 'night' ? <NightBackground /> : <IslandBackground reduceMotion={reduceMotion} />}
        ...
        <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
          ...
          <div className="bg-white/80 backdrop-blur rounded-2xl shadow-sm border p-4 mb-4 flex flex-col items-center">
            ...
            {gachaHistory[todayKey()] ? (
              ...
              <button onClick={spinGacha} className={`relative w-44 h-44 rounded-full shadow-xl overflow-hidden ${reduceMotion ? '' : 'animate-spin-slow-snap'}`}>
          ...
          <div className="flex flex-wrap items-center justify-between mb-3 gap-2">
            <div className="flex items-center gap-2">
              <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="px-3 py-1.5 rounded-xl border bg-white/70 hover:bg-white">← 前月</button>
              <div className="font-semibold">{currentMonth.getFullYear()}年 {currentMonth.getMonth() + 1}月</div>
              <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))} className="px-3 py-1.5 rounded-xl border bg-white/70 hover:bg-white">翌月 →</button>
              <button onClick={() => setCurrentMonth(new Date())} className="px-3 py-1.5 rounded-xl border bg-white/70 hover:bg-white">今月</button>
            </div>
          ...
          <div className="space-y-6">
            ...
            {habits.map(h => (
              <HabitCard key={h.id} habit={h} days={days} currentMonth={currentMonth} onStampToday={() => stampToday(h.id)} onToggleRest={(d) => toggleRest(h.id, d)} totalPoints={totalPoints} />
            ))}
          </div>
        </main>
      ) : view === 'shop' ? (
...
// ========== サブコンポーネント ==========
function HabitCard({ habit, days, currentMonth, onStampToday, onToggleRest, totalPoints }) {
